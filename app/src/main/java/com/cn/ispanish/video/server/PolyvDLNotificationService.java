package com.cn.ispanish.video.server;
...
import com.cn.ispanish.video.download.PolyvIdDownloaderManager;
...
public class PolyvDLNotificationService extends Service {
    ...
    private void initRVAndNB(int id, String title) {
        if (id_builder.get(id) == null) {
            ...
            remoteViews.setImageViewResource(R.id.custom_icon, R.mipmap.app_logo);
            ...
            // pendingIntent
            Intent resultIntent = new Intent(this, DownloadVideoListActivity.class);
            TaskStackBuilder stackBuilder = TaskStackBuilder.create(this);
            // 要在该Activity的xml中配置其父Activity
            stackBuilder.addParentStack(DownloadVideoListActivity.class);
            ...
            builder.setContent(remoteViews).setSmallIcon(R.mipmap.app_logo).setContentIntent(resultPendingIntent);
    ...
    public void updateAllPauseNF(List<VideoDownloadInfo> infos) {
        for (VideoDownloadInfo info : infos) {
    ...
    public void updateAllStartNF(List<VideoDownloadInfo> infos) {
        for (VideoDownloadInfo info : infos) {
            int id = getId(info.getVid(), info.getBitrate(), info.getSpeed());
            int progress = 0;
            if (info.getTotal() != 0)
    ...
    public void updateUnfinishedNF(List<VideoDownloadInfo> infos, List<String> finishKey) {
        for (VideoDownloadInfo info : infos) {
            boolean flag = false;
